...
def accounts():
    try:
        ...
        client = session.client(service_name="organizations", region_name=REGION)
...
def clusters(session, region_name):
    client = session.client(service_name="eks", region_name=region_name)
...
def check_fargate(session, region_name, cluster_name):
    ...
    try:
        response = client.list_fargate_profiles(clusterName=cluster_name, maxResults=10)
...
def start_build(cluster_name, cluster_arn, node_type, account_id, region_name):
    try:
        session = boto3.session.Session()
        client = session.client(service_name="codebuild", region_name=REGION)
        build = client.start_build(
            ...
            environmentVariablesOverride=[
                {"name": "CLUSTER", "value": f"{cluster_name}", "type": "PLAINTEXT"},
                {"name": "NODE_TYPE", "value": f"{node_type}", "type": "PLAINTEXT"},
                {"name": "CLUSTER_ARN", "value": f"{cluster_arn}", "type": "PLAINTEXT"},
                {"name": "ACCOUNT_ID", "value": f"{account_id}", "type": "PLAINTEXT"},
                {"name": "REGION", "value": f"{region_name}", "type": "PLAINTEXT"},
            ],
        )
